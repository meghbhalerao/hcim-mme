...
parser.add_argument('--hp-reg', type=float, default=0.2, metavar='HPM',
                    help='hyper parameter of regularizer')
...
def train():
    ...
    for step in range(all_step):
        ...
        output = G(data)
        out1 = F1(output)
        reg_loss = regularizer(F1.fc2.weight)
        loss = criterion(out1, target) + args.hp_reg*reg_loss
