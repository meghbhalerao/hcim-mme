...
def return_dataset_test_unseen(args):
    ...
    target_dataset_unl = Imagelists_VISDA(image_set_file_test,
                                          transform=data_transforms['test'],
                                          test=True)
    class_list = return_classlist("/cbica/home/bhaleram/comp_space/random/personal/iisc_project/MME/data/txt/labeled_source_images_real.txt")
    print("%d classes in this dataset" % len(class_list))
    if args.net == 'alexnet':
        bs = 32
    else:
...
def return_dataset_stage_two(args):
    base_path = './data/txt/%s' % args.dataset
    root = './data/%s/' % args.dataset
    image_set_file_s = \
        os.path.join(base_path,
    ...
    target_loader_unl = \
        torch.utils.data.DataLoader(target_dataset_unl,
                                    ...
                                    shuffle=True, drop_last=True)
    return source_loader, target_loader, target_loader_unl, \
          target_loader_val, class_num_list, class_list
...
def return_dataset_train_eval(args):
    ...
    source_dataset = Imagelists_VISDA([image_set_file_s,image_set_file_t], root=root,
                                          transform=data_transforms['test'], test=True , multiple_files=True)
    if args.net == 'alexnet':
        bs = 32
    else:
        bs = 24
